...
/** Tracks in-flight resume operations to prevent concurrent invocations per appId. */
...
export async function resumeGenerationIfNeeded(appId: string): Promise<void> {
  ...
  // If a resume (or startGeneration) is already in progress for this app,
  // skip to avoid duplicate polling loops and duplicate error messages.
  const existing = getGeneration(appId);
  if (existing?.busy) return;
  if (resumeInFlight.has(appId)) return;
  resumeInFlight.add(appId);
